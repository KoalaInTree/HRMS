...
<div id="header"><h1></h1></div>
		...
		<div id="content">
			...
			<form id="f1" action="/HRMS/PosRequest.action" method="post" name="searchForm">
			<input type="hidden" name="pagenum" id="pagenum"/>
			<div class="container-fluid">
				<div class="row-fluid">
					<div class="control-group">
						<span class="span3">
							名称：<input id="ename" name="posname" type="text" style="width:65%" value="${svo.posname}"/>
						</span>
			...
			<div class="row-fluid">
				<div class="span12" style="padding-left:30px;">
					...
					<script type="text/javascript">
						function addJob(){
							location.href="/HRMS/jsp/addJob.jsp";
						}
					</script>
				</div>
				<div class="span12">
				...
				<form id="${p.positionid}2" action="/HRMS/setposicondition.action" method="post">
				 <input name="positionid" type="hidden" value="${p.positionid}">
				 <input name="mark" type="hidden" value="${p.mark}">
				 </form>
				 <form id="${p.positionid}1" action="/HRMS/PosInfo.action" method="post">
                 <input type="hidden" name="posname" value="${p.positionname}">  
                 </form>
                  <form id="${p.positionid}3" action="/HRMS/updateposition.action" method="post">
                 <input type="hidden" name="pid" value="${p.positionid}">  
                 <input type="hidden" name="mark" value="${p.mark}">
                 <input type="hidden" name="posname" value="${p.positionname}">
                 </form>
                                    <tr>
                                        <td>${p.positionid}</td>
                                        <td>${p.positionname}</td>
                                        <td>${p.positionlevel}</td>
                                        <td>
                                        <c:choose>
                                        	<c:when test="${p.mark==1}"> 活动 </c:when>
                                        	<c:when test="${p.mark==2}"> 撤销 </c:when>     
                                        </c:choose>                                       
                                        </td>
                                         <td id="leaveInfoId" style="display:none;">${p.positionid}</td>
                                        <td>
